...
| 解剖结构分割 | `anatomyResults[*].SegmentationMask` | 髁突、下颌分支、**上颌窦**的 Polygon 坐标 (Label: condyle_left/right, mandible_left/right, sinus_left/right) |
| **上颌窦炎症** | `maxillarySinus[*].Inflammation` | 炎症判断 (true/false)，模型输出直接透传 |
| **上颌窦分型** | `maxillarySinus[*].TypeClassification` | 上颌窦分型 (0/1)，模型输出直接透传 |
| 牙齿分割 | `toothAnalysis[*].FDI` | 牙齿编号 |
| 牙齿分割 | `toothAnalysis[*].SegmentationMask` | 牙齿的 Polygon 坐标 |
...
|------|--------------------------------------------------|
| 对称性判断 | `CondyleAssessment.OverallSymmetry`, `RamusSymmetry`, `GonialAngleSymmetry` |
...
| **关键点坐标** | `LandmarkPositions.Landmarks[*]` | 25个关键点的 Label, X, Y, Confidence, Status |
| **颈椎分割** | `Cervical_Vertebral_Maturity_Stage.Coordinates` | 颈椎分割的多边形坐标 |
| **颈椎分割** | `Cervical_Vertebral_Maturity_Stage.SerializedMask` | 颈椎分割的 RLE 编码 |
